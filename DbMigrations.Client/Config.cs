            ...
            {
                {
                    "server=", "The db server (\"Data Source\" part of the connection string. " +
                               ...
                               "string of the form 'server[:port[/service]]', e.g. 'localhost:1521/XE'",
                    s => _server = s
                },
                {
                ...
                {
                    "whatif", "Do not actually run the scripts, just report what would be executed).",
                    s => WhatIf = true
                },
                {
                ...
                },
                {
                    "providerName=", "The database provider invariant name. The default is " +
                                     ...
                                     string.Join(Environment.NewLine,
                                         SupportedDataProviders.Names.Select(n => " - " + n)),
                    s => ProviderName = s
                },
                {
                    "schema=", "Db schema for Migrations table (if different from the default for this user)",
                    s => Schema = s
                },
                {
                    "reinitialize",
                    "Will clear the database and re-run all migrations. Unless --force is specified, this is only allowed for local databases. Use with care!",
                    s => ReInit = true
                },
                {
                    "force", "When used with --reinitialize, allows to restage a remote db. Use with care!",
                    s => Force = true
                },
                {
                    "persistConfiguration", "Persist the configuration to the config file",
                    s => PersistConfiguration = true
                },
                {
                    "pre=",
                    "Comma-separated list of subfolders containing scripts to execute before migration (default none). " +
                    "It is highly recommended to only include generic (idem-potent) scripts in this folder. If you don't " +
                    "know what that means, don't use this feature.",
                    s => PreMigration = s.Split(',')
                },
                {
        ...
        static class SupportedDataProviders
        {
            public static readonly DataProvider[] DataProviders =
            {
                ...
                new DataProvider
                {
                    ...
                    AssemblyQualifiedName = "System.Data.SQLite.SQLiteFactory, System.Data.SQLite"
                },
                new DataProvider
                {
